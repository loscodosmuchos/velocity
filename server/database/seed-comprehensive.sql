-- VELOCITY Comprehensive Seed Data
-- Sample data for all charts and dashboards
-- Run: psql -d $DATABASE_URL -f server/database/seed-comprehensive.sql

-- Clear existing data (safe order - respect foreign keys)
DELETE FROM voice_qa_logs;
DELETE FROM voice_analysis_sessions;
DELETE FROM voice_contract_uploads;
DELETE FROM generation_batch_findings;
DELETE FROM generation_batch_items;
...
INSERT INTO departments (name, color, icon) VALUES
  ('IT Operations', '#06B6D4', 'cpu'),
  ('Data Science', '#A855F7', 'database'),
  ('Cloud Infrastructure', '#14B8A6', 'cloud'),
  ('QA', '#F59E0B', 'test-tube-2'),
  ('Security', '#EF4444', 'shield-alert');
...
SELECT 
  'cfao@velocity.ai' as email,
  '$2b$10$mockpassword' as password_hash,
  'Sarah' as first_name,
  'Chen' as last_name,
  'admin' as role,
  (SELECT id FROM departments WHERE name = 'IT Operations') as department_id,
  'active' as status
UNION ALL SELECT 'ben@velocity.ai', '$2b$10$mockpassword', 'Ben', 'Rodriguez', 'admin', (SELECT id FROM departments WHERE name = 'IT Operations'), 'active'
UNION ALL SELECT 'mark@velocity.ai', '$2b$10$mockpassword', 'Mark', 'Johnson', 'manager', (SELECT id FROM departments WHERE name = 'Data Science'), 'active'
UNION ALL SELECT 'wes@velocity.ai', '$2b$10$mockpassword', 'Wes', 'Thompson', 'manager', (SELECT id FROM departments WHERE name = 'Cloud Infrastructure'), 'active'
UNION ALL SELECT 'aria@velocity.ai', '$2b$10$mockpassword', 'Aria', 'Patel', 'viewer', (SELECT id FROM departments WHERE name = 'Security'), 'active'
UNION ALL SELECT 'james@velocity.ai', '$2b$10$mockpassword', 'James', 'Kim', 'analyst', (SELECT id FROM departments WHERE name = 'QA'), 'active';

-- Insert Contractors (44 total for demo authenticity)
...
('C023', 'Daniel', 'Wilson', 'daniel.wilson@contractor.io', '415-555-0123', 'Test Lab', '["manual-testing","selenium"]'::jsonb, 320000.00, 'Net 30', 'Active', '2026-05-31', (SELECT id FROM departments WHERE name = 'QA'), 135.00, 88, 97.5, 1.8),
...
-- Additional contractors for volume
...
('C030', 'Rachel', 'Martin', 'rachel.martin@contractor.io', '415-555-0130', 'Documentation Pro', '["technical-writing","api-docs"]'::jsonb, 250000.00, 'Net 45', 'Active', '2026-02-28', (SELECT id FROM departments WHERE name = 'IT Operations'), 125.00, 86, 97.0, 2.3),
...
-- Insert Purchase Orders (8 SOWs for demo authenticity)
...
('PO-2025-001', (SELECT id FROM contractors WHERE contractor_id = 'C001'), 'Backend API Development', 450000.00, 225000.00, 'Active', '2025-01-01', '2025-12-31', 'IT Operations', 'Microservices Platform', (SELECT id FROM users WHERE email = 'cfao@velocity.ai')),
('PO-2025-002', (SELECT id FROM contractors WHERE contractor_id = 'C002'), 'DevOps Infrastructure', 380000.00, 285000.00, 'Active', '2025-01-15', '2025-12-31', 'IT Operations', 'Cloud Migration', (SELECT id FROM users WHERE email = 'cfao@velocity.ai')),
('PO-2025-003', (SELECT id FROM contractors WHERE contractor_id = 'C003'), 'Security Implementation', 520000.00, 390000.00, 'Active', '2025-02-01', '2026-01-31', 'IT Operations', 'Enterprise Security', (SELECT id FROM users WHERE email = 'ben@velocity.ai')),
('PO-2025-004', (SELECT id FROM contractors WHERE contractor_id = 'C012'), 'Machine Learning Models', 520000.00, 156000.00, 'Active', '2025-03-01', '2026-02-28', 'Data Science', 'AI Initiative', (SELECT id FROM users WHERE email = 'mark@velocity.ai')),
('PO-2025-005', (SELECT id FROM contractors WHERE contractor_id = 'C017'), 'Infrastructure as Code', 440000.00, 330000.00, 'Active', '2025-01-15', '2025-12-31', 'Cloud Infrastructure', 'Automation Initiative', (SELECT id FROM users WHERE email = 'wes@velocity.ai')),
('PO-2025-006', (SELECT id FROM contractors WHERE contractor_id = 'C022'), 'QA Automation', 380000.00, 114000.00, 'Active', '2025-04-01', '2026-03-31', 'QA', 'Quality Initiative', (SELECT id FROM users WHERE email = 'james@velocity.ai')),
('PO-2025-007', (SELECT id FROM contractors WHERE contractor_id = 'C026'), 'Compliance Audit', 480000.00, 240000.00, 'Active', '2025-05-01', '2025-12-31', 'Security', 'Security Hardening', (SELECT id FROM users WHERE email = 'cfao@velocity.ai')),
('PO-2025-008', (SELECT id FROM contractors WHERE contractor_id = 'C032'), 'Business Intelligence Platform', 440000.00, 220000.00, 'Active', '2025-02-15', '2026-01-31', 'Data Science', 'Analytics Platform', (SELECT id FROM users WHERE email = 'mark@velocity.ai'));

-- Insert Timecards (90 days of data for charts)
INSERT INTO timecards (
  timecard_number, contractor_id, purchase_order_id, week_ending, 
  regular_hours, overtime_hours, hourly_rate, amount, status
)
SELECT
  'TC-' || LPAD(ROW_NUMBER() OVER(), 6, '0') as timecard_number,
  c.id,
  po.id,
  (CURRENT_DATE - (INTERVAL '1 day' * (ROW_NUMBER() OVER() % 13 * 7)))::DATE as week_ending,
  ROUND((40 + (RANDOM() * 8))::NUMERIC, 2) as regular_hours,
  ROUND((RANDOM() * 5)::NUMERIC, 2) as overtime_hours,
  c.pay_rate,
  ROUND((40 + (RANDOM() * 8)) * c.pay_rate * (1 + RANDOM() * 0.5)::NUMERIC, 2) as amount,
...
SELECT
  'INV-' || LPAD(ROW_NUMBER() OVER(), 6, '0') as invoice_number,
  po.id,
  po.contractor_id,
  ...
  CASE WHEN RANDOM() > 0.3 THEN ROUND((po.total_amount / 12 + RANDOM() * 50000)::NUMERIC, 2) ELSE 0 END as amount_paid,
  CASE WHEN RANDOM() > 0.3 THEN 'Paid' WHEN RANDOM() > 0.15 THEN 'Pending' ELSE 'Disputed' END as status
...
('payment_overdue', 'high', 'open', 'Payment Overdue', 'Invoice INV-000010 is 15 days overdue', 'invoice', 10),
...
-- Insert Contractor Defects
INSERT INTO contractor_defects (
  contractor_id, defect_type, severity, description, discovery_date, status
)
SELECT
  c.id,
  CASE WHEN RANDOM() > 0.66 THEN 'quality' WHEN RANDOM() > 0.33 THEN 'performance' ELSE 'compliance' END as defect_type,
  CASE WHEN RANDOM() > 0.66 THEN 'critical' WHEN RANDOM() > 0.33 THEN 'high' ELSE 'medium' END as severity,
  'Sample defect for contractor ' || c.contractor_id as description,
  (CURRENT_DATE - (INTERVAL '1 day' * ((ROW_NUMBER() OVER()) % 30)))::DATE as discovery_date,
  CASE WHEN RANDOM() > 0.5 THEN 'resolved' ELSE 'open' END as status
FROM contractors c
WHERE c.id <= 15
LIMIT 35;

-- Insert Procurement Budgets
INSERT INTO procurement_budgets (fiscal_year, fiscal_period, total_budget, amount_spent, utilization_percent, status)
VALUES
('2025', 'Q1', 2500000.00, 1875000.00, 75.0, 'Active'),
('2025', 'Q2', 2500000.00, 1250000.00, 50.0, 'Active'),
('2025', 'Q3', 2500000.00, 625000.00, 25.0, 'Planned'),
('2025', 'Q4', 2500000.00, 0.00, 0.0, 'Planned');
...
SELECT 
  ...
  (SELECT SUM(total_budget) FROM procurement_budgets) as total_budget;
