...
function renderClips() {
  ...
  // Create clip cards using safe DOM methods
  filteredClips.forEach(clip => {
    ...
    const clipTimestamp = document.createElement('div');
    clipTimestamp.className = 'clip-timestamp';
    clipTimestamp.textContent = formatTimestamp(clip.timestamp);
    
    const clipStats = document.createElement('div');
    clipStats.className = 'clip-stats';
    
    const durationSpan = document.createElement('span');
    durationSpan.textContent = `⏱️ ${clip.duration}s`;
    
    const wordCountSpan = document.createElement('span');
    wordCountSpan.textContent = `📝 ${clip.wordCount} words`;
    
    const modelSpan = document.createElement('span');
    modelSpan.textContent = `🤖 ${clip.model}`;
    
    clipStats.appendChild(durationSpan);
    clipStats.appendChild(wordCountSpan);
    clipStats.appendChild(modelSpan);
    
    clipMeta.appendChild(clipTimestamp);
    clipMeta.appendChild(clipStats);
    ...
    starBtn.onclick = () => toggleStar(clip.id);
    starBtn.title = clip.starred ? 'Unstar' : 'Star';
    starBtn.textContent = clip.starred ? '⭐' : '☆';
    
    const copyBtn = document.createElement('button');
    copyBtn.className = 'btn-icon';
    copyBtn.onclick = () => copyClipText(clip.id);
    copyBtn.title = 'Copy to clipboard';
    copyBtn.textContent = '📋';
    
    clipActions.appendChild(starBtn);
    clipActions.appendChild(copyBtn);
    
    clipHeader.appendChild(clipMeta);
    clipHeader.appendChild(clipActions);
    
    // Create transcription element (XSS-safe with textContent)
    const clipTranscription = document.createElement('div');
    clipTranscription.className = 'clip-transcription';
    clipTranscription.textContent = clip.transcription;
    
    clipCard.appendChild(clipHeader);
    clipCard.appendChild(clipTranscription);
    
    // Add tags if they exist (XSS-safe with textContent)
    if (clip.tags.length > 0) {
      ...
      clip.tags.forEach(tag => {
        ...
        clipTags.appendChild(tagSpan);
      });
      clipCard.appendChild(clipTags);
    }
    
    // Add click listener
    clipCard.addEventListener('click', (e) => {
      if (!e.target.closest('.btn-icon')) {
        showClipDetail(clip.id);
      }
    });
...
function showClipDetail(clipId) {
  ...
  // Helper to escape HTML special characters for use in attributes
  const escapeAttr = (str) => String(str).replace(/"/g, '&quot;').replace(/'/g, '&#39;');
  
  // Build structure with static HTML (safe parts only)
  modalBody.innerHTML = `
    <div class="clip-detail">
      ...
      <div class="clip-detail-transcription"></div>
      <div class="clip-detail-actions">
        <button class="btn btn--primary" onclick="copyClipText('${escapeAttr(clip.id)}')">
          📋 Copy Text
        </button>
        <button class="btn btn--secondary" onclick="retryTranscription('${escapeAttr(clip.id)}')">
          🔄 Retry Transcription
        </button>
        <button class="btn btn--secondary" onclick="sendToMCP('${escapeAttr(clip.id)}')">
          📤 Send to MCP
        </button>
        <button class="btn btn--secondary" onclick="deleteClip('${escapeAttr(clip.id)}')">
          🗑️ Delete
        </button>
      </div>
    </div>
  ...
  // Safely set user-controlled transcription using textContent (auto-escapes HTML)
  const transcriptionDiv = modalBody.querySelector('.clip-detail-transcription');
  transcriptionDiv.textContent = clip.transcription;
